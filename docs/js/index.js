...
(function () {
    function init() {
        ...
        if (start_zoom !== undefined)
            map.deserializeState({ zoom: start_zoom });
        ...
        const initialBaseLayerName = (startLayerValid ? start_layer : undefined) ||
        ...
        for (const name of baseLayerNames)
            baseLayers[name].visible = name === initialBaseLayerName;
        ...
        gui_layers.add(baseLayerControl, 'active', baseLayerNames).name('Base map').onChange((value) => {
            for (const name of baseLayerNames)
                baseLayers[name].visible = name === value;
        });
        ...
        setInterval(() => {
            ...
            if (currentHash === lastWrittenHash)
                return; // nothing actually changed since last tick

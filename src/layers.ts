...
export function subdomain(x: number, y: number): string {
...
function drawDebugInfo(this: Layer, map: MapWidget): void {
  ...
  ctx.fillText(
    'mem(cache)=' + formatMemoryEstimate(tsFront.estimated_bytes) + ' (every tile the LRU cache currently holds)',
    x, h - 160
  );
  ctx.fillText(
    'mem(screen)=' + formatMemoryEstimate(visibleBytes) + ' (only tiles in the current visible range)',
    x, h - 140
  );
  ...
  ctx.fillText(
    ...
    + ' hits=' + tsFront.cache_hit_count + ' (served from cache, no request to the source)',
    x, h - 100
  );
  ctx.fillText(
    'prefetch=' + tsFront.prefetch_queued_count
    + ' pending=' + tsFront.load_queue.length + ' (queued for background preload, not loaded yet)',
    x, h - 80
  );
}
...
export function mandelbrotTileRect(x: number, y: number, z: number): ComplexRect {
...
export function mandelbrotEscapeIterations(cre: number, cim: number, maxIter: number): number {
  let zre = 0;
  let zim = 0;
  let iter = 0;
...
function drawMandelbrotTile(getCtx: () => CanvasRenderingContext2D, x: number, y: number, z: number): boolean {

...
export class EventEmitter<EventMap extends Record<string, unknown> = Record<string, unknown>> {
  ...
  private _listeners: Map<keyof EventMap, EventHandler<unknown>[]> = new Map();
  ...
  once<K extends keyof EventMap>(event: K, handler: EventHandler<EventMap[K]>): () => void {
    ...
    const wrapped: EventHandler<EventMap[K]> = (data) => {
      off();
      handler(data);
    };
  ...
  fire<K extends keyof EventMap>(event: K, data: EventMap[K]): void {
    const list = this._listeners.get(event);
